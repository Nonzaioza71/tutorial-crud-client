...
class Insert extends Component {
    ...
    componentDidMount(){
        this._generateProductCode()
    }

    _generateProductCode(){
        const date = new Date()
        const string = "abcdefghijklmnopqrstuvwxyz"
        const firstChar = string[Math.floor(Math.random() * (string.length-1)) + 1]
        this.setState({
            product_no: ('PT-'+firstChar.toUpperCase()+date.getTime().toString().split("").reverse().join("")).substring(0, 10)
        })
    }

    async _onSubmit() {
        if (this._checkSubmit()) {
            const res = await product_model.insertProductByCode({
